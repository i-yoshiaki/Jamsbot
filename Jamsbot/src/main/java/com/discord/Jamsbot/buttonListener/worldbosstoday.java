...
public class worldbosstoday extends buttonListenerAbstract {

	public worldbosstoday(ButtonInteractionEvent event) {
		super(event);
	}
	...
	public void execute() {
		...
		LocalDateTime nowDatePlasTenMinutes = LocalDateTime.now();
		//LIstの場合-1なので-1する。
		...
		try (Connection conn = connector.connect(PropertyManager.getProperties(BlackDesertBossTimerDbPropertyKey.BLACKDESERT_DB_NAME.getKey()));
			...
			stmt.setString(1, dayOfWeekList.get(dayOfWeek));
			...
			if (rsNameList.isEmpty()) {
				System.out.println("ボスがみつかりません。(" + nowDatePlasTenMinutes.toString() + ")");
